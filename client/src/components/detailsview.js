...
const DetailsView = ({ contact }) => {
  ...
  const getDetails = async () => {
    try {
      const response = await fetch(
        `lists/${contact.receiver}/${contact.sender}`
      );
  ...
  const addbal = async () => {
    var newType = "";
    if (newBalance < 0) newType = "credit";
    else newType = "debit";
  ...
  return (
    <Fragment>
      <button className="primary_green dlevel1 right" onClick={openmod1}>
        History
      </button>
      <div className="modal_container" id={`id${contact.receiver}`}>
        ...
        >
          ...
          <div className="large6 mediumm8 small12 center">
            <ul
              className="page_container translist"
      ...
      <div className="modal_container" id={`aid${contact.receiver}`}>
        ...
        >
          ...
          >
            <h5 style={{ display: "inline" }}>
              Add Transaction with {contact.receiver}
            </h5>
          </div>
          <br />
          <br />
          ...
          <div className="large6 mediumm8 small12 center">
            <form className="page_container" onSubmit={onFormSubmit}>
              <input
                ...
                onClick={(e) => setSR(contact.sender, contact.receiver)}
                required
              />
              <br />
              <label>Amount to be Added/Subtracted</label>
              <br />
              <br />
              ...
              <br />
              <br />
              <button className="small primary_blue">Add</button>
              <br />
